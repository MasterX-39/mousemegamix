...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Screen Shake':
						ClientPrefs.screenshake = !ClientPrefs.screenshake;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Screen Shake':
				daText = "If unchecked, the screen won't shake in some Songs.";
			case 'No Antimash':
				daText = "If checked, disables the antimash system.";
			case 'Hide HUD':
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Screen Shake':
						daValue = ClientPrefs.screenshake;
					case 'No Antimash':
